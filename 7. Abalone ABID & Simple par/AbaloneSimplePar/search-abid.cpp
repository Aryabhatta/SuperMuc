...
{    
    ...
    for( int k =1; k< NumTask; k++)
    {
        MPI_Send(&exitval,1, MPI_INT,k, 10, MPI_COMM_WORLD ); // value
    }
...
int ABIDStrategy::alphabeta(int depth, int alpha, int beta)
{
    ...
    while (1) 
    {
        ...
        if( rank == 0 && depth == 0  )
        {
            ...
            iDepthSearchS[(NoMoves%(NumTask-1))] = (doDepthSearch)?1:0;
        ...
        if( rank == 0 && depth == 0 && (NoMoves%(NumTask-1)==(NumTask-2)) )
        {
            // get value from slaves
            for( int k =1; k< NumTask; k++ )
            {
                MPI_Recv(&valueS[k-1], 1, MPI_INT,k, 10, MPI_COMM_WORLD, &stat );
